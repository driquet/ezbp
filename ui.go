	...
	form := huh.NewForm(
		huh.NewGroup( // Group UI elements.
			huh.NewSelect[string](). // Create a select prompt.
							Title("Boilerplate to expland"). // Set the title of the prompt.
							Options(opts...).                // Set the options for the select prompt.
							Value(&name),                    // Store the selected value in the 'name' variable.
		),
	)
...
func (u *TermUI) Select(prompt string, choices []string) (string, error) {
	...
	// Create and run a new select prompt using the huh library.
	err := huh.NewSelect[string]().
		Title(prompt). // Set the title of the prompt.
		Options(huh.NewOptions[string](choices...)...). // Set the options for the select prompt.
		Value(&value). // Store the selected value in the 'value' variable.
		Run()
	if err != nil {
...
type RofiUI struct {
	config RofiUIConfig // Holds Rofi-specific configuration from main Config
}

// NewRofiUI creates a new RofiUI instance with the given Rofi configuration.
func NewRofiUI(config RofiUIConfig) UI {
