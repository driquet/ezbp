...
```bash
go install github.com/USERNAME/ezbp@latest
```

(Replace `github.com/USERNAME/ezbp` with the actual repository path once available.)
...
(Note: Assuming a `LICENSE` file will be present in the repository).
...
*   **UI/UX Enhancements:**
    *   **Alternative UI (Rofi):** An integration with Rofi is available for a fast, keyboard-driven UI experience, configurable via `config.toml`. (See "Configuration" section for details).
    ...
    *   **Interactive Expansion:** Potentially offer a more step-by-step interactive expansion process for very complex boilerplates.
*   **Alternative Boilerplate Storage Formats (Legacy Idea):** While SQLite is the current plan, previous considerations included support for JSON or YAML as alternative local storage formats for boilerplates.
*   **Error Handling and Reporting:** Implement more granular error handling and introduce verbosity levels for command-line output to aid in debugging.
...
<!-- 
The Rofi UI Integration section below was relevant when it was a proposal.
Now that it's implemented, its key configuration details are in the main "Configuration" section,
and its mention in "UI/UX Enhancements" is updated.
Keeping the old detailed proposal commented out for historical reference if needed,
but it can be removed in a future cleanup.

### Rofi UI Integration (Proposed)
